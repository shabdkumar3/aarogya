        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                symptoms_in = gr.Textbox(
                    ...
                    placeholder="e.g. 3-day fever 102F, cough, child aged 7, lives near pond",
                    lines=4,
                )
                image_in    = gr.Image(
                    label="Upload Photo (optional — skin, wound, rash)",
            ...
            with gr.Column(scale=1):
                ...
                gr.HTML('<div style="margin-top:16px;color:#94A3B8;font-size:0.78rem;font-weight:700;letter-spacing:.05em">DIAGNOSIS HISTORY</div>')
                history_out = gr.Markdown(value="*Select a patient to view history.*")

        def do_scan(selection, symptoms, image, lang):
