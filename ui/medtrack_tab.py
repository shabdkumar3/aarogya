...
def build_medtrack_tab():
    with gr.TabItem("💊 MedTrack"):
            ...
            with gr.TabItem("📝 Log Dose"):
                with gr.Row():
                    with gr.Column():
                        ...
                        # Use Radio instead of Checkbox — avoids Gradio boolean schema bug
                        dose_status = gr.Radio(
        ...
        def load_meds(selection):
            ...
            return get_active_meds_display(pid) if pid else "*Select a patient.*"
        ...
        def do_stop(selection, med_id):
            pid  = parse_dropdown(selection)
            msg  = stop_med(med_id)
            disp = get_active_meds_display(pid) if pid else "*Select a patient.*"
            return msg, disp

        def do_log(selection, med_id, status, notes):
            pid  = parse_dropdown(selection)
            # Convert Radio string → bool for log_dose
            taken = (status == "Took")
        ...
        log_btn.click(do_log,
                      inputs=[patient_dd, dose_med_id, dose_status, dose_notes],
