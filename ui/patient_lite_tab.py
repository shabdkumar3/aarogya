...
def build_patient_lite_tab():
    with gr.TabItem("🌾 Patient Lite"):
        ...
        gr.HTML('<div class="alert-ok">Simple health Q&amp;A in your local language. Voice output available.</div>')
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                text_in    = gr.Textbox(
                    label="Type your health question / अपना सवाल लिखें",
                    placeholder="e.g. मुझे तेज बुखार है, क्या करूँ?",
                    lines=3,
                )
                audio_in   = gr.Audio(
                    label="Or record your question (voice — transcription is a stub)",
                    type="filepath",
                    sources=["microphone"],
                )
        ...
        def do_submit(lang, text, audio_path):
            # Try voice transcription (stub — returns empty)
            if audio_path and not (text and str(text).strip()):
                transcribed, _ = transcribe_audio(audio_path, lang)
                if transcribed and transcribed.strip():
                    text = transcribed

            if not text or not str(text).strip():
                return (
                    "⚠️ Please type your question (voice transcription requires faster-whisper).",
                    None, "—"
                )

            answer, model = run_patient_lite_text(str(text).strip(), lang)
